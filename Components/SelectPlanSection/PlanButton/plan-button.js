...
export default function PlanButton({icon, planName, planPrice, selectedPlan, setSelectedPlan, yearlyPlanBiling}) {
    ...
    const active = "w-full md:w-24 h-16 md:h-32 rounded-md bg-slate-100 flex items-center justify-center border border-violet-800 cursor-pointer";
    const notActive= "w-full md:w-24 h-16 md:h-32 rounded-md bg-transparent flex items-center justify-center border border-gray-300 cursor-pointer hover:border-violet-800";
    
    return(
        <div className={selectedPlan === planName ? active : notActive} onClick={() => setSelectedPlan(planName)}>
            <div className="w-4/5 h-4/5 flex flex-row md:flex-col justify-start md:justify-between items-center md:items-start gap-4">
